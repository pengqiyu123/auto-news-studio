...
@app.post("/api/admin/agent-html/targets", response_model=AgentHtmlTargetResponse)
def create_agent_html_target(payload: AgentHtmlTargetCreatePayload):
    try:
        return AgentHtmlTargetResponse(item=store.create_agent_html_target(payload))
    except ValueError as exc:
        raise _http_from_value_error(exc) from exc


@app.get("/api/admin/agent-html/targets", response_model=AgentHtmlTargetsResponse)
def list_agent_html_targets():
    return AgentHtmlTargetsResponse(items=store.list_agent_html_targets())


@app.patch("/api/admin/agent-html/targets/{target_id}", response_model=AgentHtmlTargetResponse)
def update_agent_html_target(target_id: str, payload: AgentHtmlTargetUpdatePayload):
    try:
        return AgentHtmlTargetResponse(item=store.update_agent_html_target(target_id, payload))
    except ValueError as exc:
        raise _http_from_value_error(exc) from exc


@app.post("/api/admin/agent-html/targets/{target_id}/run", response_model=AgentHtmlRunResponse)
def run_agent_html_target(target_id: str, triggered_by: str = "dashboard"):
    try:
        return AgentHtmlRunResponse(item=store.run_agent_html_target(target_id, triggered_by=triggered_by))
    except ValueError as exc:
        raise _http_from_value_error(exc) from exc


@app.post("/api/admin/agent-html/runs/batch", response_model=AgentHtmlRunsResponse)
def run_agent_html_targets_batch(payload: AgentHtmlRunBatchPayload):
    try:
        return AgentHtmlRunsResponse(items=store.run_agent_html_targets_batch(payload.target_ids, triggered_by=payload.triggered_by))
    except ValueError as exc:
        raise _http_from_value_error(exc) from exc


@app.post("/api/admin/agent-html/mainline-sync", response_model=SourceSyncResponse)
def sync_agent_html_into_mainline(payload: AgentHtmlMainlineBatchPayload):
    try:
        return store.sync_agent_html_into_mainline(payload.target_ids, triggered_by=payload.triggered_by)
    except ValueError as exc:
        raise _http_from_value_error(exc) from exc


@app.get("/api/admin/agent-html/runs", response_model=AgentHtmlRunsResponse)
def list_agent_html_runs():
    return AgentHtmlRunsResponse(items=store.list_agent_html_runs())


@app.get("/api/admin/agent-html/discovery", response_model=AgentHtmlDiscoveryResponse)
def list_agent_html_discovery(page: int = 1, page_size: int = 50):
    items, total = store.list_agent_html_discovery(page=page, page_size=page_size)
    safe_page = max(1, page)
    safe_page_size = max(1, min(page_size, 200))
...
@app.get("/api/admin/agent-html/events", response_model=AgentHtmlEventsResponse)
...
@app.get("/api/admin/agent-html/events/{event_id}", response_model=AgentHtmlEventResponse)
def get_agent_html_event(event_id: str):
    try:
        return AgentHtmlEventResponse(item=store.get_agent_html_event(event_id))
    except ValueError as exc:
        raise _http_from_value_error(exc) from exc


@app.get("/api/admin/agent-html/documents", response_model=AgentHtmlDocumentsResponse)
def list_agent_html_documents(page: int = 1, page_size: int = 50):
    items, total = store.list_agent_html_documents(page=page, page_size=page_size)
    safe_page = max(1, page)
    safe_page_size = max(1, min(page_size, 200))
...
@app.get("/api/admin/agent-html/documents/{document_id}", response_model=AgentHtmlDocumentResponse)
def get_agent_html_document(document_id: str):
    try:
        return AgentHtmlDocumentResponse(item=store.get_agent_html_document(document_id))
    except ValueError as exc:
        raise _http_from_value_error(exc) from exc


@app.post("/api/admin/agent-html/documents/{document_id}/reextract", response_model=AgentHtmlDocumentResponse)
def reextract_agent_html_document(document_id: str, triggered_by: str = "dashboard"):
    try:
        return AgentHtmlDocumentResponse(item=store.reextract_agent_html_document(document_id, triggered_by=triggered_by))
    except ValueError as exc:
        raise _http_from_value_error(exc) from exc
